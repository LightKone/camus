...
%% @doc camus peer service connections module.

-module(camus_ps_connections).
...
-type connections() :: maps:map(camus_id(), connection()).
...
-spec connect(camus_id(), node_ip(), node_port(), connections()) ->
...
-spec members(connections()) -> list(camus_id()).
members(Connections) ->
    maps:fold(
...
%%        - if not, it did :)
-spec exit(camus_id(), pid(), connections()) -> {boolean(), connections()}.
...
%% @private
-spec do_connect(camus_id(), node_ip(), node_port(), connections(), non_neg_integer()) ->
    {ok | error(), connections()}.
do_connect(_, _, _, Connections, 0) ->
    {ok, Connections};
do_connect(Id, Ip, Port, Connections0, N) ->
    Name = camus_util:connection_name(Id, N),
    case camus_ps_client:start_link(Name, Ip, Port) of
        {ok, Pid} ->
